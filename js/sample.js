...
db.rstQ("demo_pjan", {SEX: "T", GEO: "NL"}, "TIME asec") //2
    .then(rst => {
        ...
        rst.forEach(r => {$("ul").append(`<li>In ${r.TIME}: ${r.OBS_VALUE}</li>`);});//3
    }, undefined, val => {console.log(`inflight: ${val}`);});
...
var then = data => console.log(JSON.stringify(data, null, 2)); //callback to display information to console
...
function sample3() {
    ...
    db.dataQ("demo_pjan", {SEX: ["M", "F"], GEO:"NL"})
        .then(() => {
            var rstM = db.rst("demo_pjan", {SEX: ["M"], GEO:"NL"}, "TIME asec"), rstF = db.rst("demo_pjan", {SEX: ["F"], GEO:"NL"}, "TIME asec"), rst = [];
            for (let i = 0; i<rstM.length; i++) {
            ...
            rst.forEach(r => {$("div#info").append(`<p>In ${r.TIME}: ${r.OBS_VALUE}</p>`);});
        }, undefined, val => {console.log(`inflight${val}`);});
}
...
function sample4() {
    db.dataQ("nrg_100a", {PRODUCT: ["0000", "2000"], INDIC_NRG: "B_100900", GEO: ["DE", "LU", "BE"]})
        .then(
        rst => {console.log(`additionally fetched:   ${JSON.stringify(rst)}`);},
        e => {console.log(`error:   ${e.message}`);},
        f => {console.log(`inflight:   ${f}`);}
    );
    db.dataQ("nrg_100a", {PRODUCT: ["0000", "2000"], INDIC_NRG: "B_100900", GEO: ["DE", "LU", "BE", "NL"]})
        .then(
        rst => {console.log(`additionally fetched 2: ${JSON.stringify(rst)}`);},
        e => {console.log(`error 2: ${e.message}`);},
        f => {console.log(`inflight 2: ${f}`);}
    );
}